...
describe('db/options: actions creators', () => {
	...
	describe('setOptionValue()', () => {
		test('should create valid action', () => {
			...
			const actual = actions.setOptionValue('rendering', 'transposeValue', 3);

			expect(actual).toEqual(expected);
		});

		test('should throw if not given context', () => {
			const throwingFn = () => actions.setOptionValue(undefined, 'transposeValue', 3);
		...
		test('should throw if not given key', () => {
			const throwingFn = () => actions.setOptionValue('rendering', undefined, 3);
